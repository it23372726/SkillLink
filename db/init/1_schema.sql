...
CREATE TABLE dbo.TutorPosts (
  ...
  CONSTRAINT FK_TutorPosts_Users
    FOREIGN KEY (TutorId) REFERENCES dbo.Users(UserId)
    ON DELETE CASCADE
);
GO
...
CREATE TABLE dbo.TutorPostParticipants (
  ...
  -- Keep cascade only via PostId to avoid multiple cascade path through Users->TutorPosts->Participants
  CONSTRAINT FK_TPP_Posts
    FOREIGN KEY (PostId) REFERENCES dbo.TutorPosts(PostId)
    ON DELETE CASCADE,
  CONSTRAINT FK_TPP_Users
...
CREATE TABLE dbo.Friendships (
  ...
  -- Allow cascade on one side only to avoid multiple cascade path
...
CREATE TABLE dbo.PostReactions (
  ...
  INDEX IX_PostReactions_Post (PostType, PostId),
...
CREATE TABLE dbo.PostComments (
  ...
  INDEX IX_PostComments_Post (PostType, PostId),
...
CREATE TABLE dbo.AcceptedRequests (
  ...
  -- Keep cascade via RequestId; block direct cascade from Users -> AcceptedRequests to avoid multiple cascade path
...
CREATE TABLE dbo.Sessions (
  ...
  -- Cascade only via RequestId; block direct User -> Sessions cascade
